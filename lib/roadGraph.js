...
function bindToLineSegment(fromCoords, lineStartNode, lineEndNode) {
    ...
    if (Math.sqrt(distanceToLineStart * distanceToLineStart - h * h) < 5) next_nodes.push(ResultObj(lineStartNode, distanceToLineStart));else {
        for (var i = 0, n = lineEndNode.next_nodes.length, node_obj = lineEndNode.next_nodes[0]; i < n; node_obj = lineEndNode.next_nodes[++i]) {
