...
function bindToLineSegment(fromCoords, lineStartNode, lineEndNode) {
    ...
    if (Math.pow(lineLength, 2) < Math.abs(Math.pow(distanceToLineStart, 2) - Math.pow(distanceToLineEnd, 2))) {
    ...
    var tmp = Math.sqrt(Math.pow(distanceToLineStart, 2) - Math.pow(h, 2));
