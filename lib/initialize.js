...
var vehicleRoadTypes = ['motorway', 'motorway_link', 'trunk', 'trunk_link', 'primary', 'primary_link', 'secondary', 'secondary_link', 'tertiary', 'tertiary_link', 'unclassified', 'unclassified_link', 'residential', 'service', 'living_street', 'track'];
...
function initialize(osm_graph_elements_original, type) {
    ...
    osm_graph_elements.forEach(function (item) {
        switch (item.type) {
            case "way":
                {
                    if (type === _roadGraph.GraphTypes.pedestrian || vehicleRoadTypes.includes(item.tags.highway)) {
