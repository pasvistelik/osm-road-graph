...
function initialize(osm_graph_elements) {
    ...
    for (var j = 0, m = ways.length, way = ways[0]; j < m; way = ways[++j]) {
        var is_oneway = way.tags.oneway == "yes";
        for (var _i2 = 1, _n2 = way.nodes.length + 1, previous_node = null, current_node = binaryFind(nodes, getFindingNodeFunc(way.nodes[0])), next_node = way.nodes[1] ? binaryFind(nodes, getFindingNodeFunc(way.nodes[1])) : null; _i2 < _n2; previous_node = current_node, current_node = next_node, next_node = way.nodes[++_i2] ? binaryFind(nodes, getFindingNodeFunc(way.nodes[_i2])) : null) {
            if (is_oneway) {
                // if road is one-way only
                if (next_node) {
                    current_node.next_nodes.push({
                        node: next_node,
                        distance: ~~(0, _geoCoordsDistance2.default)({ lat: current_node.lat, lng: current_node.lon }, { lat: next_node.lat, lng: next_node.lon })
                    });
                    //console.log()
                }
                continue;
            ...
            if (previous_node) {
                current_node.next_nodes.push({
                    node: previous_node,
                    distance: ~~(0, _geoCoordsDistance2.default)({ lat: current_node.lat, lng: current_node.lon }, { lat: previous_node.lat, lng: previous_node.lon })
                });
            }
        }
    }
